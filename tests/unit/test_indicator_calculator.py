...
class TestIndicatorCalculatorSMA:
    """SMA (단순 이동평균) 테스트"""
    ...
    def test_sma_calculation(self):
        ...
        calc = IndicatorCalculator()
        result = calc._calc_sma(df, length=3)

        assert len(result) == len(df)
        # 처음 2개는 NaN (window 부족)
        assert pd.isna(result.iloc[0:2]).all()
        # 3번째부터 계산됨
...
class TestIndicatorCalculatorEMA:
    """EMA (지수 이동평균) 테스트"""
    ...
    def test_ema_calculation(self):
        ...
        calc = IndicatorCalculator()
        result = calc._calc_ema(df, length=3)

        assert len(result) == len(df)
        # EMA는 초기화 기간(처음 1-2행)에 NaN이 있을 수 있음
        # 초기화 이후의 값들은 계산됨
        assert not pd.isna(result.iloc[2:]).any()  # 3번째부터는 계산됨

    def test_ema_responsive_to_changes(self):
        """EMA가 가격 변화에 반응"""
        df1 = pd.DataFrame({
            'adj_close': [100] * 10
        })
        df2 = pd.DataFrame({
            'adj_close': [100] * 5 + [110] * 5  # 중간에 상승
        })

        calc = IndicatorCalculator()
        ema1 = calc._calc_ema(df1, length=5)
        ema2 = calc._calc_ema(df2, length=5)

        # df2의 EMA가 df1의 EMA보다 높아야 함
        assert ema2.iloc[-1] > ema1.iloc[-1]


class TestIndicatorCalculatorMACD:
    """MACD 테스트"""
    ...
    def test_macd_returns_three_columns(self):
        ...
        calc = IndicatorCalculator()
        result = calc._calc_macd(df, fast=12, slow=26, signal=9)
